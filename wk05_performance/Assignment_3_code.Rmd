...
> *Guidance.* The original data set has the levels in the factor variable `default` ordered as 'No' and 'Yes'. The value of interest is 'Yes', therefore it must appear in the first position. To do this, one must relevel the variable `default`.
...
> *Guidance.* Additional metrics would generally include metrics such as: accuracy, Cohen's Kappa, precision, recall, sensitivity, specificity, and F-measure. 
...
> *Guidance.* This is not the only way to write the formula, an alternative is: `default ~ student + balance + income`.
...
> *Guidance.* Sequence of neighbors could also be written as `c(2, 4, 8, 16, 32, 64, 128, 256)`.
...
> *Guidance.* The control function `control_resamples(save_pred = TRUE)` is needed to answer questions further below.
...
> *Guidance.* Notice that precision is not defined for 256 neighbors. This is because no positive cases are predicted and, therefore, in the calculation of precision the denominator is 0. As well, this is due to the fact that the problem has imbalanced classes (the positive class is a small minority). The model is not useful for our purposes when this happens because we care about the positive cases. 
...
> *Guidance.* Similar to the case above, precision is not defined for higher penalty values. 
...
> *Guidance*. The idea is that CV will give better error estimates than trainin. In the response, a comparison should be made between CV and test errors.
...
> *Guidance.*  The idea would be to enhance prediction quality for the 'Yes' (defaulted) case.