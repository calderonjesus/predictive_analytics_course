...
> Guidance: the hyperparameters `trees` and `mtry` must be set to be tuned. Some submissions may use a different engine than ranger.
...
rf_base_wf <- 
  workflow() %>%
  add_recipe(rf_base_rec) %>%
  add_model(rf_mod)

rf_enh_wf <- 
  workflow() %>%
  add_recipe(rf_enh_rec) %>%
  add_model(rf_mod)
```
...
> Guidance: at least one of the ranges should be adjusted. At least 20 levels (overall) should be tested. The function `finalize()` must be run for `mtry`.
...
> Guidance: in this case, a low number of features and trees maximize f-measure. The model appears to be unstable with intermediate number of features and we may be better off with a different approach.

```{r}
rf_base_res %>%
  collect_metrics() %>%
  ggplot(aes(x = trees, y = mean,
             colour = as_factor(mtry))) +
  geom_point() +
  geom_line(aes(group = as_factor(mtry))) +
  facet_wrap(~.metric, scales = "free_y") +
  theme_minimal() +
  guides(colour = guide_legend(title = 'N. Features')) +
...
> Guidance: Similar observations apply as in the case above. Performance is slightly increased by the enhanced model.
...
```{r}
save(rf_base_wf, rf_base_res, file = "./res/boost_base_res.Rda")
save(rf_enh_wf, rf_enh_res, file = "./res/boost_enh_res.Rda")
```